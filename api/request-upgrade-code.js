...
module.exports = async (req, res) => {
  ...
  try {
    const membership = await findEligibleSubscription(email);
    if (!membership) {
      return res.status(400).json({
        error: 'Zu dieser E-Mail-Adresse wurde kein aktives MNQ-Abo gefunden.'
      });
    }

    const code = String(crypto.randomInt(100000, 1000000));
    const payload = Buffer.from(JSON.stringify({ ...membership, code, exp: Date.now() + 10 * 60 * 1000 })).toString('base64url');
    ...
    const emailResponse = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        to: [{ email }],
        subject: 'Dein Code für das Bundle-Upgrade',
        htmlContent: `<p>Dein Bestätigungscode für das ChampVision Bundle-Upgrade lautet:</p><p style="font-size:28px;font-weight:bold;letter-spacing:5px">${code}</p><p>Der Code ist 10 Minuten gültig.</p>`
      })
    });
